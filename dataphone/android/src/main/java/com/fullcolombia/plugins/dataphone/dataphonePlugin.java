...
public class dataphonePlugin extends Plugin implements ResultIntegrationSDK {
     ...
     * concedido cuando se intente imprimir. En Android 11+ (scoped storage) no
     * aplica.
     */
    ...
    public void print(PluginCall call) {
        try {
            ...
            if (o != null && copia == 0) {
                ...
                valuesToSend
                        .add(TEXT + ", COPIA PARA ADMIN EDS Y CONTROL DE FLOTAS ," + FONT_NORMAL + "," + ALIGN_CENTER);
            }
            ...
            if (res != null && (tipo == 1 || tipo == 2)) {
                ...
                // Mostrar número sin prefijo si tipo 3 o 4, o simplemente siempre mostrar el
                // número
            ...
            if (general != null) {
                ...
                valuesToSend
                        .add(TEXT + ",Fecha: " + general.optString("fecha", "") + "," + FONT_NORMAL + "," + ALIGN_LEFT);
                valuesToSend
                        .add(TEXT + ",Hora: " + general.optString("hora", "") + "," + FONT_NORMAL + "," + ALIGN_LEFT);
            }
            ...
            valuesToSend.add(
                    TEXT + ",Atendido por: " + json.optString("islero", "") + "," + FONT_NORMAL + "," + ALIGN_LEFT);
            valuesToSend.add(TEXT + ", ," + FONT_NORMAL + "," + ALIGN_CENTER);
            ...
            if (copia == 0 && !URL_TO_ENCODE.isEmpty()) {
                try {
                    ...
                    Log.e("ResultActivity",
                            "Error generando QR del recibo original, se imprime sin QR: " + qrError.getMessage());
                }
            }
            ...
            IntegrationPeripherialSDK sdkPeripherals = IntegrationPeripherialSDK.Companion
                    .getSmartPosInstancePeripherals();
    ...
    public void printShiftClosureTicket(PluginCall call) {
        try {
            ...
            valuesToSend.add(TEXT + ",CIERRE DE TURNO No: " + shift.optInt("id_shift", 0) + "," + FONT_NORMAL + ","
                    + ALIGN_LEFT);
                ...
                for (int i = 0; i < surt.length(); i++) {
                    ...
                    for (String cKey : Arrays.asList("c1", "c2")) {
                        ...
                        for (int j = 0; j < man.length(); j++) {
                            ...
                            valuesToSend.add(
                                    TEXT + ",Vol. Inicial : " + volInicialStr + "," + FONT_NORMAL + "," + ALIGN_LEFT);
                            valuesToSend.add(
     ...
     * - Retorna la(s) línea(s) que debes agregar a valuesToSend para imprimir el
     * QR.
     *
    ...
    private java.util.List<String> generateQrForPrint(String url) throws Exception {
        ...
        com.google.zxing.common.BitMatrix matrix = writer.encode(url, com.google.zxing.BarcodeFormat.QR_CODE, SIZE,
                SIZE, hints);
    ...
    public void printQrTicket(PluginCall call) {
        try {
            ...
            com.google.zxing.common.BitMatrix matrix = writer.encode(URL_TO_ENCODE,
     ...
     * java.io.File base = (ctx.getExternalFilesDir(null) != null) ?
     * ctx.getExternalFilesDir(null) : ctx.getFilesDir();
    ...
    // API ≤28: escribe en Pictures/ (requiere WRITE_EXTERNAL_STORAGE en tiempo de
    // ejecución).
    /*
     * private static android.net.Uri savePngToGalleryCompat(Context ctx, byte[]
     * pngBytes, String displayName) throws Exception {
     * if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
     ...
     * java.io.File pictures =
     * android.os.Environment.getExternalStoragePublicDirectory(android.os.
     ...
     * android.content.Intent scanIntent = new
    ...
    public void resultActivity(@NonNull ActivityResult activityResult) {
         ...
         * Log.d("ResultActivity", key + " : " + (value != null ? value.toString() :
         * "null"));
         * }
         * } else {
